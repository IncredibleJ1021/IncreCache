...
void printResults(const std::string& testName, int capacity,
    ...
    for (size_t i = 0; i < hits.size(); ++i) {
        ...
        std::cout << (i < names.size() ? names[i]
                  ...
                  << hitRate << "% ";
        // 添加具体命中次数和总操作次数
        std::cout << "（" << hits[i] << "/" << get_operations[i] << "）"
                  << std::endl;
    }
...
void testHotDataAccess() {
    ...
    const int CAPACITY = 20;       // 缓存总量
    const int OPERTIONS = 500000;  // 总操作次数
    const int HOT_KEYS = 20;       // 热点数据数量
    const int COLD_KEYS = 5000;    // 冷数据数量

    IncreCache::ILruCache<int, std::string> lru(CAPACITY);
    IncreCache::ILfuCache<int, std::string> lfu(CAPACITY);
    IncreCache::IArcCache<int, std::string> arc(CAPACITY);

    // 为 LRU-K 设置合适的参数：
    // - 主缓存容量与其它算法相同
    // - 历史记录容量设为可能访问的所有键数量
    // - k = 2 表示数据被访问 2 次后才会进入缓存，适合区分热点和冷数据
    IncreCache::ILruKCache<int, std::string> lruk(CAPACITY,
                                                  HOT_KEYS + COLD_KEYS, 2);
    IncreCache::ILfuCache<int, std::string> lfuAging(CAPACITY, 20000);

    std::random_device rd;
    std::mt19937 gen(rd());

    // 基类指针指向派生类对象，添加 LFU-Aging
    std::array<IncreCache::ICachePolicy<int, std::string>*, 5> caches = {
        &lru, &lfu, &arc, &lruk, &lfuAging};
    std::vector<int> hits(5, 0);
    std::vector<int> get_operations(5, 0);
    std::vector<std::string> names = {"LRU", "LFU", "ARC", "LRU-K",
                                      "LFU-Aging"};
    ...
    for (int i = 0; i < caches.size(); ++i) {
        ...
        for (int op = 0; op < OPERTIONS; ++op) {
            ...
            if (isPut) {
                // 执行 put 操作
                std::string value = "value" + std::to_string(key) + "_v" +
...
void testLoopPattern() {
    ...
    IncreCache::ILruKCache<int, std::string> lruk(CAPACITY, LOOP_SIZE * 2, 2);
    IncreCache::ILfuCache<int, std::string> lfuAging(CAPACITY, 3000);
    ...
    for (int i = 0; i < caches.size(); ++i) {
        ...
        for (int op = 0; op < OPERATIONS; ++op) {
            ...
            if (isPut) {
                // 执行 put 操作，更新数据
                std::string value = "loop" + std::to_string(key) + "_v" +
                                    std::to_string(op % 100);
                caches[i]->put(key, value);
            } else {
                // 执行 get 操作并记录命中情况
...
void testWorkloadShift() {
    ...
    IncreCache::ILfuCache<int, std::string> lfu(CAPACITY);
    IncreCache::IArcCache<int, std::string> arc(CAPACITY);
    IncreCache::ILruKCache<int, std::string> lruk(CAPACITY, 500, 2);
    IncreCache::ILfuCache<int, std::string> lfuAging(CAPACITY, 10000);
    ...
    for (int i = 0; i < caches.size(); ++i) {
        ...
        for (int op = 0; op < OPERATIONS; ++op) {
            ...
            int key;
            if (op <
                PHASE_LENGTH) {  // 阶段1:热点访问 - 热点数量5，使热点更集中
                key = gen() % 5;
            } else if (op < PHASE_LENGTH * 2) {  // 阶段2:大范围随机 - 范围
                                                 // 400，更适合 30 大小的缓存
                key = gen() % 400;
            } else if (op <
                       PHASE_LENGTH * 3) {  // 阶段3:顺序扫描 - 保持 1000 个键
                key = (op - PHASE_LENGTH * 2) % 100;
            } else if (op < PHASE_LENGTH *
                                4) {  // 阶段4:局部性随机 - 优化局部性区域大小
                // 产生 5 个局部区域，每个区域大小为 15
                // 个键，与缓存大小20接近但略小
                int locality = (op / 800) % 5;  // 调整为 5 个局部区域
                key = locality * 15 + (gen() % 15);
            } else {  // 阶段5:混合访问 - 增加热点访问比例
                int r = gen() % 100;
                if (r < 40) {  // 40% 概率访问热点（从 30% 增加）
                    key = gen() % 5;
                } else if (r < 70) {  // 30% 概率访问中等范围
                    key = 5 + (gen() % 45);
                } else {  // 30% 概率访问大范围（从 40% 减少）
                    key = 50 + (gen() % 350);  // 大范围也相应缩小
                }
            }
            if (isPut) {
                // 执行写操作
                std::string value = "value" + std::to_string(key) + "_p" +
                                    std::to_string(phase);
                caches[i]->put(key, value);
            } else {
                // 执行读操作并记录命中情况
